(function() {
  ...
  attachMethods(String.prototype, {
    ...
    ljust: function(w) {
      var l = this.length;
      return l < w ? (this + new Array(w - l).join(" ")) : this;
    }
  });
  ...
  jQuery.fn.extend({
    outerHTML: function() {
      return this.length > 0 ? this[0].outerHTML : null;
    },

    toString: function() {
      return 'jQuery({0}) [{1} items]'.format(this.selector, this.length);
    }
  });


  var _oldArrayToString = Array.prototype.toString;
  Array.prototype.toString = function() {
    return "[" + _oldArrayToString.call(this) + "]";
  };
  ...
  _.my = {
    ...
    win: function(body) {
      var w = window.open("about:blank", "_blank", 640, 800, "resizable=1");
      w.document.write("<html><head><style>body { font: 12px Consolas; }</style></head><body>");
      w.document.write(_my.htmlescape(body));
...
var STYLE_PRINT = 3;
var STYLE_ERROR = 4;
var STYLE_PROPERTIES = 5;
var STYLE_TABCOMPLETE = 6;

var STYLES = 'input normalOutput message print error propList tabcomplete'.split(' ');

function text(style, message) {
  ...
  // print([s, m, o, Object.type(o), o.length]);
  // var text = message.format.apply(message, data);

  return '<div class="line {cls}">{text}</div>'.format({
    cls: STYLES[style],
    text: message.format.apply(message, data)
  });
}
...
jQuery(function($) {
  ...
  if (opener && !opener.closed) {
    $('h3').after(text(STYLE_MESSAGE, 'Using bookmarklet version of shell: commands will run in opener\'s context.'));
    // $('h3').after("Using bookmarklet version of shell: commands will run in opener's context.", "message");
...
function recalculateInputHeight() {
  var rows = _in.value.split(/\n/).length + 1 // prevent scrollbar flickering in Mozilla
...
function println(s, type) {
  ...
  newdiv.appendChild(document.createTextNode(String(s)));
...
var shellCommands = {
  ...
  history: function history() {
    $.each(histList.slice(0, -1), function(i, v) {
      printWithRunin(i, v, "propList");
    });
  },
  ...
  props: function props(obj, onePerLine) {
    if (obj === null) {
    ...
    Namespace.prototype = {
      addItem: function(k, o) {
        try {
          ...
          this.unreachables.push(k);
        }
      },

      toString: function() {
        return this.obj.toString() + " (lvl " + this.level + ")";
      },

      write: function() {
        var self = this, op = onePerLine ? "\n\n": "", ip = onePerLine ? "\n": ", ";

        function hd(n) { return n + new Array(self.level + 1).join(" of prototype"); }
        function bd(f) { return op + self[f].sort().join(ip) + op; }

        if (this.methods.length) printWithRunin(hd("Methods"), bd("methods"), "propList");
        if (this.fields.length) printWithRunin(hd("Fields"), bd("fields"), "propList");
        if (this.unreachables.length) printWithRunin(hd("Unreachables"), bd("unreachables"), "propList");
      }
    };

    var namespaces = [];
    var curObj, j, i; // loop variables, several used multiple times
    var protoLevels = 0;

    for (curObj = obj; curObj; curObj = curObj.__proto__) {
      namespaces.push(new Namespace(protoLevels, curObj));
      ...
      try {
        for (curObj = obj; curObj; curObj = curObj.__proto__) {
          if (curObj.hasOwnProperty(key)) {
            namespaces[protoLevel].addItem(key, curObj);
            break;
          }
          ++protoLevel;
        }
        ...
        namespaces[0].addItem(key, obj);
      }
    }

    for (j = 0; j < protoLevels; ++j) {
      var curns = namespaces[j];
      curns.write();
    }
  },
  ...
  blink: function blink(node) {
    if (!node || node.nodeType !== Document.ELEMENT_NODE) {
      throw new TypeError("blink: argument must be an element.");
    }
...
function hist(up) {
  ...
  else // down
  {
    if (histPos < L - 1) {
      ...
      if (_in.value) {
        histList[histPos] = _in.value; ++histPos;
        _in.value = "";
      }
    }
  }
...
function tabcomplete() {
  ...
  if (caret) {
    //dump("----\n");
    var dotpos, spacepos, complete, obj, objname;
    ...
    for (var a in obj) {
      //a = a.toString();
      //XXX: making it lowercase could help some cases, but screws up my general logic.
      ...
      if (a.substr(0, complete.length) === complete) {
        ...
        if (bestmatch === null) {
          ...
          for (i = 0; i < Math.min(bestmatch.length, a.length); i++) {
    ...
    if (bestmatch !== "") {
      var sstart = 1 + dotpos === caret ? spacepos : dotpos;
...
/* Experimental! But seems ok!

function evaljs(js, win) {
  // Double-encode js for safety with e.g. quotes, wrap in closure
  var jsfunc = "(function(){" + escape(escape(js)) + "})()",
    jsurl = "javascript:(function(){eval(unescape('" + jsfunc + "'));})()",
    w = win || window;

  w.location.href = jsurl;
}

*/

function go(s) {
  question = s || _in.value;
  processedQuestion = question;
  var m;

  if (question.match(/^clear|mathHelp|scope$/)) {
    processedQuestion += "()";
  }
  else if (m = question.match(/^(\?\??)\s*(.*)$/)) {
    processedQuestion = "props(" + m[2] + ", " + (m[1] === "??" ? "true" : "false") + ")";
  }
  ...
  histList[histList.length - 1] = processedQuestion;
  ...
  if (!_win.hasOwnProperty('Shell')) {
    initTarget(); // silent
  }

  // Evaluate Shell.question using _win's eval (this is why eval isn't in the |with|, IIRC).
  _win.location.href = "javascript:try{ Shell.printAnswer(eval('with(Shell._scope) with(Shell.shellCommands) {' + Shell.processedQuestion + String.fromCharCode(10) + '}')); } catch(er) { Shell.printError(er); }; setTimeout(Shell.refocus, 0); void 0";
}
